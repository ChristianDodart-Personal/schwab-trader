...
export const GLOSSARY: Record<string, GlossaryEntry> = {
  ...
  market_value: {
    ...
    howCalculated: "Sum over open lots of shares × the latest quote. When every lot has a known cost this equals cost basis + unrealized P/L; shares with no known cost add to value but to neither of those.",
  ...
  harvestable: {
    term: "Harvestable",
    oneLiner: "Profit you could lock in right now by selling every last position that's in the green.",
    howItWorks: "Counts each holding whose newest lot (the one a LIFO sale retires) is above its cost. Bulk Sell sells whole shares only and needs a live Schwab quote, so what it actually books can be slightly less. A position whose newest lot has no known cost is left out.",
    howCalculated: "Sum over last positions in profit of (current price − that lot's buy price) × its shares.",
    source: "computed",
  ...
  buy_dip: {
    ...
    howItWorks: "The drop depends on how deep the ladder already is: each rung range has its own % in the Rules tab (deeper rungs wait for bigger drops). When deployment scaling is on, that % is multiplied by the tier for how much of your own money is invested.",
    howCalculated: "Next trigger = last position's buy price × (1 − the drop % for the next rung × the deployment multiplier).",
    source: "computed",
    related: ["last_position", "ladder_rung", "sell_target"],
  },
  sell_target: {
    term: "Sell target",
    oneLiner: "The price at which the app suggests selling a lot for profit.",
    howItWorks: "Every lot has its own target. The dashboard's SELL chip uses the newest lot's target, because a sale retires that lot first (LIFO).",
    howCalculated: "Default mode: the lot's buy price + (your flat $ gain ÷ its shares). Other mode: buy price × (1 + your %). A target saved on a lot overrides both.",
    source: "computed",
    related: ["last_position", "harvestable"],
  },
  simple_view: {
    term: "Simple view",
  ...
  lot_age: {
    term: "Age",
    oneLiner: "How long ago this lot was bought.",
    howItWorks: "Measured per lot, not per position. Not the same as Hold days, which runs from a trade's first buy to its final sell.",
    howCalculated: "Calendar days from the lot's buy date to today.",
    source: "computed",
    related: ["hold_days", "ladder_rung"],
  },
  lilo_pct: {
    term: "LILO %",
    oneLiner: "How far the price sits above (or below) your newest position's buy price.",
    howItWorks: "Your newest position is the one a sale retires first (LIFO), so this is how far that sale is from breaking even. Negative means the newest lot is underwater.",
    howCalculated: "Current price ÷ newest lot's buy price − 1. Blank when the newest lot has no known cost.",
    source: "computed",
    related: ["last_position", "sell_target", "buy_dip"],
  },
  avg_monthly: {
    term: "Avg monthly",
    oneLiner: "Your average realized profit per month on this ticker.",
    howItWorks: "All-time within the trade history the app has, not year-to-date.",
    howCalculated: "Realized profit on this ticker ÷ days since its first closed trade × 30.",
    source: "computed",
    related: ["realized_pl"],
  },
  portfolio_pct: {
    term: "Portfolio %",
    oneLiner: "This position's share of the money you have invested.",
    howItWorks: "Based on cost, and cash is not included, so it's a share of invested money rather than of the whole account. Informational only: there's no cap.",
    howCalculated: "This position's cost ÷ the total cost of all open positions.",
    source: "computed",
    related: ["invested", "cost_basis"],
  },
  position_day_change: {
    term: "Day P/L",
    oneLiner: "How much this holding's value changed today.",
    howItWorks: "Schwab's own per-position day change when Schwab is reachable, which counts same-day buys and intraday sells. Otherwise the app estimates it from the live quote and today's fills. Deposits and withdrawals never enter it.",
    howCalculated: "Estimate: value now + today's sell proceeds − today's buy cost − (shares held at the open × yesterday's close).",
    source: "hybrid",
    related: ["day_change", "unrealized_pl"],
  },
  progressive_tax: {
